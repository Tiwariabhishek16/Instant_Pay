...
@Service
public class RemitterRegistrationServiceImpl implements RemitterRegistrationService {

    @Autowired
    private RestTemplate restTemplate; // RestTemplate for making HTTP requests

    @Override
    public RemitterRegistrationResponse registerRemitter(RemitterRegistrationRequest request, String authCode, String clientId, String clientSecret, String endPointIp, String outletId) {
        // Set up the headers
        ...
        HttpEntity<RemitterRegistrationRequest> entity = new HttpEntity<>(request, headers);

        // Define the external API URL
        String url = "https://api.instantpay.in/fi/remit/out/domestic/remitterRegistration";

        // Make the HTTP POST request to the external API
        ResponseEntity<RemitterRegistrationResponse> responseEntity =
                restTemplate.exchange(url, HttpMethod.POST, entity, RemitterRegistrationResponse.class);
